...
const { liriklagu, quotemaker, jadwalTv, fb } = require("./lib/functions");
  ...
  try {
    ...
    switch (command) {
            ...
            if (
                ...
                fetch("https://trace.moe/api/search", {
                    ...
                    .then((resolt) => {
                        ...
                        teks += `➸ *Similarity* : ${(similarity * 100).toFixed(
                            1
                        )}%\n`;
        ...
        case "!mentionall":
            ...
            let hehe =
                "╔══✪〘 Mention All requested by " + pushname + " 〙✪══\n";
        ...
        case "!nsfw":
            ...
            if (command == "!meme") subr = "memes";
                ...
                if (!isNsfw && isGroupMsg)
                    return client.reply(
                        chatId,
                        "NSFW not enabled in this group",
                        id
                    );
                subr = "nsfwmemes";
            }
        ...
        case "!yt":
            ...
            writeStream.on('open',()=>ytdl(yturl).pipe(writeStream));
            writeStream.on('finish',() =>{
              client.sendFile(chatId,path,ytId+'.mp4','').then(()=>fs.unlink(path));
            });
            break;
        default:
